...
// Mock data for testing the UI before the API is ready
...
export const mockServerStats: ServerStatsData = {
  ...
  total_players: mockServers.reduce((acc, server) => acc + server.status.players, 0),
  servers_by_status: {
    alive: mockServers.filter(server => server.status.status === 'alive').length,
    degraded: mockServers.filter(server => server.status.status === 'degraded').length,
    down: mockServers.filter(server => server.status.status === 'down').length
  }
};
